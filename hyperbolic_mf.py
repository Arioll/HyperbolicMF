...
#             weighted_diff = p - relations[:, row_ind]
#         V_grad[:, :-1] = 2 * weighted_diff.T @ self.U[:, :-1]
#         assert not np.any(np.isnan(V_grad[:, -1]))
...
#         assert not np.any(np.isnan(V_grad[:, -1]))
#         grad = self.project_gradient_onto_tangent_space(V_grad, V_matrix[row_ind, :])
#         assert not np.any(np.isnan(grad[:, -1]))
#         grad = self.exp_mu(V_matrix[row_ind, :], -lr * grad)
#         assert not np.any(np.isnan(grad[:, -1]))
#         V_matrix[row_ind, :] = grad
...
#                 if weights is not None:
#                     w = weights[batch_size * i:batch_size * (i + 1), batch_size * j:batch_size * (j + 1)]
#                     first_term = (w.multiply(self.backend.log(1 + self.backend.exp(-d_L)) + rel.toarray() * d_L)).sum()
#                 else:
...
#         try:
#             assert not np.any(np.isnan(norm_x[:, -1]))
#         except:
#             print((x * x * mask).sum(axis=1))
#             print(self.backend.sqrt((x * x * mask).sum(axis=1)))
#             raise Exception()
...
#     # HAGD algorithm from paper
#     def fit(self, relation_matrix, weight_matrix, num_epochs, learning_rate=1e-3, decomposition_rank=5, batch_size=10, seed=42):
#         if seed is not None:
...
#             iterator.set_description(f'Epoch: {epoch + 1} \t Loss: {float(loss):.4f}')
#             # if len(losses) > 0 and abs(loss - losses[-1]) < 1e-3:
#             #     learning_rate *= 0.1
#             losses.append(float(loss))
#             # print(self.U[:, -1].min(), self.V[:, -1].min())
#             # print('-' * 100)
...
class HyperbolicMF:
    ...
    def update_U_row(self, U_matrix, relations, weights, row_ind, lr):
        ...
        if weights is not None:
            ...
            # sums = (w > 0).sum(axis=1, keepdims=True)
            w[w == 0] = 1
            # w = w / sums
            weighted_diff = w * (p - relations[row_ind, :])
        else:
            weighted_diff = p - relations[row_ind, :]
        U_grad = 2 * weighted_diff @ self.V
        positive_inds = U_matrix[:, -1] > 1
        inner_pos_inds = self.backend.intersect1d(self.backend.where(positive_inds)[0], row_ind)
        pos_U = U_matrix[inner_pos_inds, -1]
        U_grad[inner_pos_inds - row_ind[0], -1] -= 2 * a_U * self.backend.arccosh(pos_U) / self.backend.sqrt(pos_U ** 2 - 1)
        U_grad[inner_pos_inds - row_ind[0], -1] -= (d - 1) * (pos_U * self.backend.arccosh(pos_U) - self.backend.sqrt(pos_U ** 2 - 1)) / ((pos_U ** 2 - 1) * self.backend.arccosh(pos_U))
        grad = self.project_gradient_onto_tangent_space(U_grad, U_matrix[row_ind, :])
        grad = self.exp_mu(U_matrix[row_ind, :], -lr * grad)
        U_matrix[row_ind, :] = grad
    ...
    def update_V_row(self, V_matrix, relations, weights, row_ind, lr):
        ...
        p = self.predict(self.backend.arange(n_users), row_ind)
        if weights is not None:
            # sums = (weights > 0).sum(axis=1, keepdims=True)[:, row_ind]
            w = weights[:, row_ind].toarray()
            w[w == 0] = 1
            # w = w / sums
            weighted_diff = w * (p - relations[:, row_ind])
            ...
            weighted_diff = p - relations[:, row_ind]
        V_grad = 2 * weighted_diff.T @ self.U
        positive_inds = V_matrix[row_ind, -1] > 1
        inner_pos_inds = self.backend.intersect1d(self.backend.where(positive_inds)[0], row_ind)
        pos_V = V_matrix[inner_pos_inds, -1]
        V_grad[inner_pos_inds - row_ind[0], -1] -= 2 * a_V * self.backend.arccosh(pos_V) / self.backend.sqrt(pos_V ** 2 - 1)
        V_grad[inner_pos_inds - row_ind[0], -1] -= (d - 1) * (pos_V * self.backend.arccosh(pos_V) - self.backend.sqrt(pos_V ** 2 - 1)) / ((pos_V ** 2 - 1) * self.backend.arccosh(pos_V))
        grad = self.project_gradient_onto_tangent_space(V_grad, V_matrix[row_ind, :])
        grad = self.exp_mu(V_matrix[row_ind, :], -lr * grad)
        V_matrix[row_ind, :] = grad
    ...
    def compute_loss(self, relations, weights, batch_size):
        ...
        a_V = 1 / (2 * self.sigma_V ** 2)
        loss = 0
        U_n_batches = int(self.backend.ceil(self.U.shape[0] / batch_size))
        V_n_batches = int(self.backend.ceil(self.V.shape[0] / batch_size))
        for i in range(U_n_batches):
            U_inds = self.backend.arange(batch_size * i, min(batch_size * (i + 1), self.U.shape[0]))
            for j in range(V_n_batches):
                V_inds = self.backend.arange(batch_size * j, min(batch_size * (j + 1), self.V.shape[0]))
